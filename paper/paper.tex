...
\affiliation{University of Chicago, \todo{need details here, and ORCID above}}
...
\item The spectrograph is consistently and repeatably calibrated in a
  ...
  calibrated near-perfectly consistently across epochs (exposures).
...
derivatives are column vectors, and $C^{-1}$ is the covariance matrix
describing the true variance and covariances of the noise process.

...Now speak to the more restrictive assumptions others have made but
say that we agree in the end.
\begin{itemize}
\item Butler equation \todo{See notes in Hogg's red book for 2017-12-01.}
\item Bouchy equation
\item Figueira (or Lovis \& Fischer)
\end{itemize}

... Now note that the \CRLB\ is saturated with a maximum likelihood estimator under the correct model.

... Now note that using a wrong spectrum template will necessarily move you away from the bound. Bad!

In conclusion: the saturate the CRLB, we need to construct a very
accurate spectral template, an accurate tellurics template, and
maximize the likelihood. Given that theory sucks, this probably means
data-driven.
